...
//
// Env vars supported:
// - HENRY_API_KEY (required)
// - HENRY_ENV: 'sandbox' | 'production' (optional; defaults to 'sandbox')
// - HENRY_SDK_BASE_URL (optional override for base URL)

const apiKey = process.env.HENRY_API_KEY;
if (!apiKey) {
  throw new Error("Henry SDK: missing API key. Set HENRY_API_KEY in your environment.");
}

const environment = (process.env.HENRY_ENV as "sandbox" | "production" | undefined) ?? undefined;
const baseURL = process.env.HENRY_SDK_BASE_URL ?? undefined;
